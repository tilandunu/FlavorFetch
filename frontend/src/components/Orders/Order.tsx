...
const Order: React.FC<DeliveryOrderProps> = (props) => {
  const { _id, customerId, deliveryAddress, note, estimatedTime } =
    props.deliveryOrder;
  ...
  const validateForm = () => {
    ...
    if (specialCharRegex.test(deliveryNote)) {
      ...
      toast.error("Note cannot contain special characters", {
        position: "top-center",
      });
      return false;
    }
    ...
    if (isNaN(Number(deliveryEstimatedTime))) {
      ...
      toast.error("Estimated time should be a valid number", {
        position: "top-center",
      });
      return false;
    }
